//ポップアップウインドウ作成関数
function createWindow(dictList,windowMsg,{width='auto',height='auto',top='20px',color='rgb(80,80,160)'}){
  //ウインドウパーツの定義
  ...
  myWindow.style.border='1px solid '+color;
  ...
  optionBar.style.backgroundColor=color;
...
function createInputs(type, //入力形式
  ...
  if(type==='text'){
    ...
    elem.style.padding='5px';
    //イベント設定
    year.addEventListener('focus',onFocusInput,false);
    year.addEventListener('blur',onBlurInput,false);
    month.addEventListener('focus',onFocusInput,false);
    month.addEventListener('blur',onBlurInput,false);
    day.addEventListener('focus',onFocusInput,false);
    day.addEventListener('blur',onBlurInput,false);
    return elem;
...
function confirmWindow(msg,trueFunc,falseFunc){
  const myTrueFunc=function(e){
    trueFunc(e);
    closeWindow(e);
  }
  const myFalseFunc=function(e){
    falseFunc(e);
    closeWindow(e);
  }
  createWindow([
...
function alertWindow(msg){
  createWindow([
    ...
    {'type':'endFlex','options':{}}
  ],'警告!',{'width':'300px','top':'20px','color':'rgb(200,60,60)'});
}
